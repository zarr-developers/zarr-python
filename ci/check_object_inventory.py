...
def validate_urls(
    ...
    print("\nValidating URLs...")
    results = []
    stats = {"total": 0, "matched": 0, "redirected": 0, "missing": 0, "external": 0}
    ...
    for url in sorted(stable_urls):
        stats["total"] += 1
        result = {"url": url, "status": "UNKNOWN", "new_url": None, "note": ""}
...
def print_report(results: list[dict], stats: dict[str, int]):
    ...
    print(f"  ✅ Matched (OK):         {stats['matched']}")
    ...
    if internal_total > 0:
        ...
        print(f"\n📈 Coverage: {coverage:.1f}% ({accessible}/{internal_total})")
    ...
    if format_changes:
        print(f"\n🔄 URL FORMAT CHANGES ({len(format_changes)})")
        print("-" * 80)
    ...
    # Final verdict
    if stats["missing"] == 0:
        print("✅ SUCCESS: All URLs from stable docs are accessible in new build!")
    else:
        print(f"⚠️  WARNING: {stats['missing']} URLs are not accessible in new build")
