...
def test_vlen_string(
    ...
    # test round trip
    b = Array.open(sp)
    # mypy resolves `Array.open`'s `Self` return to a single constrained metadata
    # type, so it wrongly thinks these statements are unreachable; they run fine.
    assert isinstance(b.metadata, ArrayV3Metadata)  # type: ignore[unreachable]  # needed for mypy
    assert np.array_equal(data, b[:, :])  # type: ignore[unreachable]
