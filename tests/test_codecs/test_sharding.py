...
def test_sharding_huge_grid_cardinality(monkeypatch: pytest.MonkeyPatch) -> None:
    """Counting chunks in sparse large arrays must not allocate a per-shard table."""
    from zarr.core.chunk_grids import ChunkGrid

    array = zarr.create_array({}, shape=(10**12,), chunks=(2,), shards=(3,), dtype="u1")

    def materialized_sizes(grid: ChunkGrid) -> tuple[tuple[int, ...], ...]:
        raise AssertionError("cardinality tried to materialize every shard")

    monkeypatch.setattr(ChunkGrid, "chunk_sizes", property(materialized_sizes))
    assert array.cdata_shape == (666666666667,)
