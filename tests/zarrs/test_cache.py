...
import zarr
import zarr.zarrs  # registers the "zarrs" CrudBackend
...
async def test_distinct_metadata_distinct_entries(tmp_path: Path) -> None:
    ...
    await read_chunk(meta_a, store, "a", (0, 0), engine="zarrs")
    await read_chunk(meta_b, store, "b", (0, 0), engine="zarrs")
...
async def test_cache_keyed_on_root_not_just_metadata(tmp_path: Path) -> None:
    ...
    out1 = await read_chunk(meta, s1, "a", (0, 0), engine="zarrs")
    out2 = await read_chunk(meta, s2, "a", (0, 0), engine="zarrs")
...
async def test_cache_reflects_writes_through_store(tmp_path: Path) -> None:
    ...
    store = await LocalStore.open(root=tmp_path / "s")
    meta = _meta(store)
    await read_chunk(meta, store, "a", (0, 0), engine="zarrs")  # caches the Array
    new = np.full((4, 4), 99, dtype="uint16")
    await write_chunk(meta, store, "a", (0, 0), new, engine="zarrs")  # write via (cached) Array
    out = await read_chunk(meta, store, "a", (0, 0), engine="zarrs")
