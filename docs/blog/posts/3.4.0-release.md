...
`zarr` 3.4.0 is out! This post covers the highlights of the latest version of `zarr`. We will also introduce the new subpackages as we seek to make `zarr` more modular. See the full [release notes](../../release-notes.md) for the per-PR breakdown, or keep reading to catch the digested version.
