...
class IntDimIndexer:
    ...
    def __iter__(self) -> Iterator[ChunkDimProjection]:
        ...
        chunk_start = self.chunk_grid.get_chunk_start(chunk_coords)
        ...
        chunk_shape = self.chunk_grid.get_chunk_shape(chunk_coords)
...
class SliceDimIndexer:
    ...
    def __iter__(self) -> Iterator[ChunkDimProjection]:
        ...
        nchunks = self.chunk_grid.chunks_per_dim(self.dim)
        ...
        if self.start == 0:
            ...
            dim_chunk_ix_from = self.chunk_grid.array_index_to_chunk_coord(start_index)[self.dim]
        ...
        if self.stop == 0:
            ...
            dim_chunk_ix_to = self.chunk_grid.array_index_to_chunk_coord(end_index)[self.dim] + 1
...
class BoolArrayDimIndexer:
    ...
    def __init__(
        ...
        nchunks = chunk_grid.chunks_per_dim(dim)
        ...
        for dim_chunk_ix in range(nchunks):
            ...
            chunk_start = chunk_grid.get_chunk_start(chunk_coords)
            chunk_shape = chunk_grid.get_chunk_shape(chunk_coords)
...
class IntArrayDimIndexer:
    ...
    def __init__(
        ...
        dim_sel_chunk = chunk_grid.array_indices_to_chunk_dim(dim, dim_sel)
...
class BlockIndexer(Indexer):
    ...
    def __init__(
        ...
        dim_indexers = []
        for dim, (dim_sel, dim_len) in enumerate(zip(selection_normalized, shape, strict=True)):
            dim_numchunks = chunk_grid.chunks_per_dim(dim)
            ...
            if is_integer(dim_sel):
                ...
                chunk_start_pos = chunk_grid.get_chunk_start(chunk_coords)
                chunk_shape_here = chunk_grid.get_chunk_shape(chunk_coords)
                ...
                start_pos_tuple = chunk_grid.get_chunk_start(start_chunk_coords)
                ...
                stop_pos_tuple = chunk_grid.get_chunk_start(stop_chunk_coords)
                stop_chunk_shape = chunk_grid.get_chunk_shape(stop_chunk_coords)
...
class CoordinateIndexer(Indexer):
    ...
    def __init__(
        ...
        if shape == ():
            ...
            cdata_shape = chunk_grid.get_chunk_grid_shape()
        ...
        chunks_multi_index_broadcast = tuple(
            chunk_grid.array_indices_to_chunk_dim(dim, selection_broadcast[dim])
