...
class RectilinearChunkGrid(ChunkGrid):
    ...
    def __init__(self, *, chunk_shapes: Sequence[Sequence[int]], array_shape: ShapeLike) -> None:
        """
    ...
    def from_dict(  # type: ignore[override]
        cls, data: dict[str, JSON], *, array_shape: ShapeLike
    ) -> Self:
        """
        ...
        return cls(chunk_shapes=chunk_shapes_expanded, array_shape=array_shape)
    ...
    def to_dict(self) -> dict[str, JSON]:
        ...
        Examples
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[10, 10, 10, 10, 10, 10], [5, 5, 5, 5, 5]], array_shape=(60, 25))
    ...
    def update_shape(self, new_shape: tuple[int, ...]) -> Self:
        ...
        -------
        Self
            A new RectilinearChunkGrid instance with updated chunk shapes and array_shape
        ...
        Examples
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[10, 20], [15, 15]], array_shape=(30, 30))
        >>> grid.update_shape((50, 40))  # Grow both dimensions
        RectilinearChunkGrid(_array_shape=(50, 40), chunk_shapes=((10, 20, 20), (15, 15, 10)))

        >>> grid = RectilinearChunkGrid(chunk_shapes=[[10, 20, 30], [25, 25]], array_shape=(60, 50))
        >>> grid.update_shape((25, 30))  # Shrink first dimension
        RectilinearChunkGrid(_array_shape=(25, 30), chunk_shapes=((10, 20), (25, 25)))
        ...
        return type(self)(chunk_shapes=tuple(new_chunk_shapes), array_shape=new_shape)
    ...
    def _validate_array_shape(self) -> None:
        """
        ...
        """
        if len(self._array_shape) != len(self.chunk_shapes):
            raise ValueError(
                f"array_shape has {len(self._array_shape)} dimensions but "
                f"chunk_shapes has {len(self.chunk_shapes)} dimensions"
            )

        for axis, (arr_size, axis_chunks) in enumerate(
            zip(self._array_shape, self.chunk_shapes, strict=False)
        ):
    ...
    def get_chunk_start(self, chunk_coord: tuple[int, ...]) -> tuple[int, ...]:
        ...
        Examples
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[2, 2, 2], [3, 3]], array_shape=(6, 6))
        >>> grid.get_chunk_start((0, 0))
        (0, 0)
        >>> grid.get_chunk_start((1, 1))
        (2, 3)
        """
    ...
    def get_chunk_shape(self, chunk_coord: tuple[int, ...]) -> tuple[int, ...]:
        ...
        Examples
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[2, 3, 1], [4, 2]], array_shape=(6, 6))
        >>> grid.get_chunk_shape((0, 0))
        (2, 4)
        >>> grid.get_chunk_shape((1, 0))
        (3, 4)
        """
        # Validate chunk coordinates are in bounds
        for axis, (coord, axis_chunks) in enumerate(
    ...
    def get_chunk_slice(self, chunk_coord: tuple[int, ...]) -> tuple[slice, ...]:
        """
        ...
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[2, 2, 2], [3, 3]], array_shape=(6, 6))
        >>> grid.get_chunk_slice((0, 0))
        (slice(0, 2, None), slice(0, 3, None))
        >>> grid.get_chunk_slice((1, 1))
        (slice(2, 4, None), slice(3, 6, None))
        """
        start = self.get_chunk_start(chunk_coord)
        shape = self.get_chunk_shape(chunk_coord)
    ...
    def array_index_to_chunk_coord(self, array_index: tuple[int, ...]) -> tuple[int, ...]:
        ...
        (0, 0)
        >>> grid.array_index_to_chunk_coord((2, 0))
        (1, 0)
        >>> grid.array_index_to_chunk_coord((5, 5))
        (2, 1)
        """
        # Validate array index is in bounds
        for axis, (idx, size) in enumerate(zip(array_index, self._array_shape, strict=False)):
            if not (0 <= idx < size):
    ...
    def chunks_in_selection(self, selection: tuple[slice, ...]) -> Iterator[tuple[int, ...]]:
        """
        ...
        >>> list(grid.chunks_in_selection(selection))
        [(0, 0), (0, 1), (1, 0), (1, 1), (2, 0), (2, 1)]
        """
        # Normalize slices and find chunk ranges for each axis
        chunk_ranges = []
        for axis, (sel, size) in enumerate(zip(selection, self._array_shape, strict=False)):
            ...
            start_chunk = self.array_index_to_chunk_coord(
                tuple(start if i == axis else 0 for i in range(len(self._array_shape)))
            )[axis]

            # stop-1 is the last index we need
            end_chunk = self.array_index_to_chunk_coord(
                tuple(stop - 1 if i == axis else 0 for i in range(len(self._array_shape)))
            )[axis]
    ...
    def chunks_per_dim(self, dim: int) -> int:
        ...
        Examples
        --------
        >>> grid = RectilinearChunkGrid(chunk_shapes=[[10, 20], [5, 5, 5]], array_shape=(30, 15))
        >>> grid.chunks_per_dim(0)  # 2 chunks along axis 0
        2
        >>> grid.chunks_per_dim(1)  # 3 chunks along axis 1
        3
        """
