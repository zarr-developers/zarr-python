...
class ChunksType(tuple[Any, ...], ABC):  # noqa: SLOT001
    """
    Abstract base class for chunk specifications.

    All chunk types inherit from both tuple (for backward compatibility)
    and this ABC (for common interface and isinstance checks).

    Subclasses must implement dimension_names property and provide
    appropriate __new__ methods.
    """

    @property
    @abstractmethod
    def dimension_names(self) -> tuple[str | None, ...] | None:
        """Dimension names for this chunk specification, if available."""
        ...
...
class RegularChunks(ChunksType):
    """
    Chunk specification for arrays with uniform chunk sizes.

    Behaves like a tuple of integers, where each element represents
    the chunk size along that dimension. Provides named access when
    dimension_names are available.

    Examples
    --------
    >>> chunks = RegularChunks((10, 20), dimension_names=("lat", "lon"))
    >>> chunks[0]
    10
    >>> chunks.lat  # Named access
    10
    >>> dict(chunks)
    {'lat': 10, 'lon': 20}
    >>> tuple(chunks)
    ...
    True
    >>> isinstance(chunks, ChunksType)
    True
    """

    def __new__(
        cls, data: tuple[int, ...], *, dimension_names: tuple[str | None, ...] | None = None
    ) -> Self:
        """
        Create a new RegularChunks instance.

        Parameters
        ----------
        data : tuple[int, ...]
            Chunk sizes for each dimension
        dimension_names : tuple[str | None, ...] | None, optional
            Names for each dimension. If provided, enables named attribute
            access and dict conversion.

        Returns
        -------
        Self
            A new RegularChunks instance
        """
        instance = super().__new__(cls, data)
        object.__setattr__(instance, "_dimension_names", dimension_names)
        return instance
    ...
    def dimension_names(self) -> tuple[str | None, ...] | None:
        """Dimension names for this chunk specification, if available."""
        return cast(
            tuple[str | None, ...] | None, object.__getattribute__(self, "_dimension_names")
        )

    def __getattr__(self, name: str) -> int:
        """Enable named access like chunks.lat when dimension_names are set."""
        dim_names = cast(
            tuple[str | None, ...] | None, object.__getattribute__(self, "_dimension_names")
        )
        if dim_names is None:
            raise AttributeError("No dimension names available")

        try:
            idx = dim_names.index(name)
            return cast(int, self[idx])
        except ValueError:
            raise AttributeError(f"No dimension named {name!r}") from None

    def keys(self) -> Iterator[str]:
        """Return dimension names, enabling dict(chunks) conversion."""
        dim_names = object.__getattribute__(self, "_dimension_names")
        if dim_names is None:
            raise ValueError("Cannot convert to dict without dimension_names")

        for name in dim_names:
            if name is None:
                raise ValueError("Cannot convert to dict with unnamed dimensions")
            yield name

    def values(self) -> Iterator[int]:
        """Return chunk sizes, enabling dict(chunks) conversion."""
        return iter(self)

    def items(self) -> Iterator[tuple[str, int]]:
        """Return (name, size) pairs, enabling dict(chunks) conversion."""
        dim_names = object.__getattribute__(self, "_dimension_names")
        if dim_names is None:
            raise ValueError("Cannot convert to dict without dimension_names")

        for name, size in zip(dim_names, self, strict=True):
            if name is None:
                raise ValueError("Cannot convert to dict with unnamed dimensions")
            yield (name, size)
    ...
    def __repr__(self) -> str:
        dim_names = object.__getattribute__(self, "_dimension_names")
        if dim_names:
            return f"RegularChunks({tuple(self)}, dimension_names={dim_names})"
        return f"RegularChunks({tuple(self)})"


class RectilinearChunks(ChunksType):
    """
    ...
    Examples
    --------
    >>> chunks = RectilinearChunks(((10, 20, 30), (5, 5)), dimension_names=("lat", "lon"))
    >>> chunks[0]
    (10, 20, 30)
    >>> chunks.lat  # Named access
    (10, 20, 30)
    >>> dict(chunks)
    {'lat': (10, 20, 30), 'lon': (5, 5)}
    >>> chunks.nchunks
    (3, 2)
    >>> chunks.total_chunks
    6
    ...
    def __new__(
        ...
        *,
        ...
        """
        Create a new RectilinearChunks instance.

        Parameters
        ----------
        data : tuple[tuple[int, ...], ...]
            Variable chunk sizes for each dimension
        dimension_names : tuple[str | None, ...] | None, optional
            Names for each dimension. If provided, enables named attribute
            access and dict conversion.
        ...
        -------
        Self
            A new RectilinearChunks instance
        """
        instance = super().__new__(cls, data)
        object.__setattr__(instance, "_dimension_names", dimension_names)
        return instance
    ...
    def nchunks(self) -> tuple[int, ...]:
        ...
        -------
        tuple[int, ...]
            Count of chunks along each dimension

        Examples
        --------
        >>> chunks = RectilinearChunks(((10, 20, 30), (5, 5)))
        >>> chunks.nchunks
        (3, 2)
        """
        return tuple(len(dim_chunks) for dim_chunks in self)
    ...
    def total_chunks(self) -> int:
        ...
        -------
        int
            Product of nchunks across all dimensions
        ...
        """
        result = 1
        for dim_chunks in self:
            result *= len(dim_chunks)
        return result

    def get_chunk_size(self, dim: int, chunk_idx: int) -> int:
        """
        ...
        dim : int
            Dimension index
        chunk_idx : int
            Index of the chunk along that dimension
        ...
        --------
        >>> chunks = RectilinearChunks(((10, 20, 30), (5, 5)))
        >>> chunks.get_chunk_size(0, 1)
        20
        """
        return cast(int, self[dim][chunk_idx])
    ...
    def __getattr__(self, name: str) -> tuple[int, ...]:
        ...
        try:
            idx = dim_names.index(name)
            return cast(tuple[int, ...], self[idx])
        except ValueError:
            raise AttributeError(f"No dimension named {name!r}") from None
    ...
    def values(self) -> Iterator[tuple[int, ...]]:
        """Return chunk size tuples, enabling dict(chunks) conversion."""
        return iter(self)

    def items(self) -> Iterator[tuple[str, tuple[int, ...]]]:
        """Return (name, sizes) pairs, enabling dict(chunks) conversion."""
        dim_names = object.__getattribute__(self, "_dimension_names")
        if dim_names is None:
            raise ValueError("Cannot convert to dict without dimension_names")

        for name, sizes in zip(dim_names, self, strict=True):
            if name is None:
                raise ValueError("Cannot convert to dict with unnamed dimensions")
            yield (name, sizes)
    ...
    def __repr__(self) -> str:
        dim_names = object.__getattribute__(self, "_dimension_names")
        if dim_names:
            return f"RectilinearChunks({tuple(self)}, dimension_names={dim_names})"
        return f"RectilinearChunks({tuple(self)})"
