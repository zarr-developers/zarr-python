...
try:
    ...
    from zarr.v2.codecs import Blosc
...
def init_array(
    ...
    Initialize an array store::
        ...
        >>> from zarr.v2.storage import init_array, KVStore
        >>> store = KVStore(dict())
...
def _init_array_metadata(
    ...
    if not overwrite:
        ...
        if contains_group(store, path, explicit_only=False):
...
class MemoryStore(Store):
    ...
    This is the default class used when creating a group. E.g.::
        ...
        >>> g = zarr.v2.group()
        >>> type(g.store)
        <class 'zarr.v2.storage.MemoryStore'>
...
class DirectoryStore(Store):
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.DirectoryStore('data/array.zarr')
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
    ...
    Store a group::
        ...
        >>> store = zarr.v2.DirectoryStore('data/group.zarr')
        >>> root = zarr.v2.group(store=store, overwrite=True)
...
class NestedDirectoryStore(DirectoryStore):
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.NestedDirectoryStore('data/array.zarr')
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
    ...
    Store a group::
        ...
        >>> store = zarr.v2.NestedDirectoryStore('data/group.zarr')
        >>> root = zarr.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
...
class ZipStore(Store):
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.ZipStore('data/array.zip', mode='w')
    ...
    Store a group::
        ...
        >>> store = zarr.v2.ZipStore('data/group.zip', mode='w')
        >>> root = zarr.v2.group(store=store)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    method is called on leaving the context, e.g.::
        ...
        >>> with zarr.v2.ZipStore('data/array.zip', mode='w') as store:
        ...     z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store)
        ...     z[...] = 42
    ...
    e.g.::
        ...
        >>> store = zarr.v2.ZipStore('data/example.zip', mode='w')
        >>> z = zarr.v2.zeros(100, chunks=10, store=store)
        >>> # first write OK
        ... z[...] = 42
    ...
    ``z[...] = ...`` or create an array from existing data via :func:`zarr.v2.array`.
    ...
    Zarr archive. You may find it easier to create such a Zip file with ``7z``, e.g.::
        ...
        7z a -tzip archive.zarr.v2.zip archive.zarr/.
...
class DBMStore(Store):
    ...
    **open_kwargs
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.DBMStore('data/array.db')
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarr.v2.DBMStore('data/group.db')
        >>> root = zarr.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
    ...
    ``close()`` method is called on leaving the context, e.g.::
        ...
        >>> with zarr.v2.DBMStore('data/array.db') as store:
        ...     z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        ...     z[...] = 42
        ...     # no need to call store.close()
    ...
    Berkeley DB database can be used::
        ...
        >>> import bsddb3
        >>> store = zarr.v2.DBMStore('data/array.bdb', open=bsddb3.btopen)
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()
...
class LMDBStore(Store):
    ...
    **kwargs
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.LMDBStore('data/array.mdb')
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarr.v2.LMDBStore('data/group.mdb')
        >>> root = zarr.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
    ...
    ``close()`` method is called on leaving the context, e.g.::
        ...
        >>> with zarr.v2.LMDBStore('data/array.mdb') as store:
        ...     z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        ...     z[...] = 42
        ...     # no need to call store.close()
...
class LRUStoreCache(Store):
    ...
    The example below wraps an S3 store with an LRU cache::
        ...
        >>> cache = zarr.v2.LRUStoreCache(store, max_size=2**28)
        >>> root = zarr.v2.group(store=cache)  # doctest: +REMOTE_DATA
...
class SQLiteStore(Store):
    ...
    **kwargs
    ...
    Store a single array::
        ...
        >>> import zarr
        >>> store = zarr.v2.SQLiteStore('data/array.sqldb')
        >>> z = zarr.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarr.v2.SQLiteStore('data/group.sqldb')
        >>> root = zarr.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
...
class ConsolidatedMetadataStore(Store):
    ...
    See :func:`zarr.v2.convenience.consolidate_metadata` for how to create this
    single metadata key.
    ...
    :func:`zarr.v2.convenience.consolidate_metadata` should be called again and the class
