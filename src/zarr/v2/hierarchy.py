...
class Group(MutableMapping[str, Any]):
    ...
    def __getitem__(self, item):
        ...
        >>> g1['foo']
        <zarr.v2.hierarchy.Group '/foo'>
        >>> g1['foo/bar']
        <zarr.v2.hierarchy.Group '/foo/bar'>
        >>> g1['foo/bar/baz']
        <zarr.v2.core.Array '/foo/bar/baz' (100,) float64>
    ...
    def groups(self):
        ...
        ...     print(n, type(v))
        bar <class 'zarr.v2.hierarchy.Group'>
        foo <class 'zarr.v2.hierarchy.Group'>
    ...
    def arrays(self, recurse=False):
        ...
        ...     print(n, type(v))
        baz <class 'zarr.v2.core.Array'>
        quux <class 'zarr.v2.core.Array'>
    ...
    def visitvalues(self, func):
        ...
        <zarr.v2.hierarchy.Group '/bar'>
        <zarr.v2.hierarchy.Group '/bar/baz'>
        <zarr.v2.hierarchy.Group '/bar/quux'>
        <zarr.v2.hierarchy.Group '/foo'>
    ...
    def visit(self, func):
        ...
        <zarr.v2.hierarchy.Group '/aaa/bbb/ccc/aaa'>
    ...
    def visititems(self, func):
        ...
        ('bar', <zarr.v2.hierarchy.Group '/bar'>)
        ('bar/baz', <zarr.v2.hierarchy.Group '/bar/baz'>)
        ('bar/quux', <zarr.v2.hierarchy.Group '/bar/quux'>)
        ('foo', <zarr.v2.hierarchy.Group '/foo'>)
        >>> g3.visititems(print_visitor)
        ('baz', <zarr.v2.hierarchy.Group '/bar/baz'>)
        ('quux', <zarr.v2.hierarchy.Group '/bar/quux'>)
    ...
    def create_dataset(self, name, **kwargs):
        ...
        synchronizer : zarr.v2.sync.ArraySynchronizer, optional
            Array synchronizer.
        ...
        ...                        chunks=(1000, 1000))
        >>> d1
        <zarr.v2.core.Array '/foo' (10000, 10000) float64>
        >>> d2 = g1.create_dataset('bar/baz/qux', shape=(100, 100, 100),
        ...                        chunks=(100, 10, 10))
        >>> d2
        <zarr.v2.core.Array '/bar/baz/qux' (100, 100, 100) float64>
    ...
    def require_dataset(self, name, shape, dtype=None, exact=False, **kwargs):
        ...
        Other `kwargs` are as per :func:`zarr.v2.hierarchy.Group.create_dataset`.
    ...
    def empty_like(self, name, data, **kwargs):
        """Create an array. Keyword arguments as per
        :func:`zarr.v2.creation.empty_like`."""
    ...
    def zeros_like(self, name, data, **kwargs):
        """Create an array. Keyword arguments as per
        :func:`zarr.v2.creation.zeros_like`."""
    ...
    def ones_like(self, name, data, **kwargs):
        """Create an array. Keyword arguments as per
        :func:`zarr.v2.creation.ones_like`."""
    ...
    def full_like(self, name, data, **kwargs):
        """Create an array. Keyword arguments as per
        :func:`zarr.v2.creation.full_like`."""
...
def group(
    ...
    *,
    ...
    Create a group with a different store::
        ...
        >>> store = zarr.v2.DirectoryStore('data/example.zarr')
        >>> g = zarr.v2.group(store=store, overwrite=True)
        >>> g
        <zarr.v2.hierarchy.Group '/'>
...
def open_group(
    ...
    *,
    ...
    --------
    >>> import zarr
    >>> root = zarr.v2.open_group('data/example.zarr', mode='w')
    >>> foo = root.create_group('foo')
    >>> bar = root.create_group('bar')
    >>> root
    <zarr.v2.hierarchy.Group '/'>
    >>> root2 = zarr.v2.open_group('data/example.zarr', mode='a')
    >>> root2
    <zarr.v2.hierarchy.Group '/'>
    >>> root == root2
    True
