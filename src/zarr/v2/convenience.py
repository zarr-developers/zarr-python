...
def open(store: StoreLike = None, mode: str = "a", *, path=None, **kwargs):
    ...
    **kwargs
        Additional parameters are passed through to :func:`zarr.v2.creation.open_array` or
        :func:`zarr.v2.hierarchy.open_group`.

    Returns
    -------
    z : :class:`zarr.v2.core.Array` or :class:`zarr.v2.hierarchy.Group`
        Array or group, depending on what exists in the given store.

    See Also
    --------
    zarr.v2.creation.open_array, zarr.v2.hierarchy.open_group
    ...
    Storing data in a directory 'data/example.zarr' on the local file system::
        ...
        >>> zw = zarr.v2.open(store, mode='w', shape=100, dtype='i4')  # open new array
        >>> zw
        <zarr.v2.core.Array (100,) int32>
        >>> za = zarr.v2.open(store, mode='a')  # open existing array for reading and writing
        >>> za
        <zarr.v2.core.Array (100,) int32>
        >>> zr = zarr.v2.open(store, mode='r')  # open existing array read-only
        >>> zr
        <zarr.v2.core.Array (100,) int32 read-only>
        >>> gw = zarr.v2.open(store, mode='w')  # open new group, overwriting previous data
        >>> gw
        <zarr.v2.hierarchy.Group '/'>
        >>> ga = zarr.v2.open(store, mode='a')  # open existing group for reading and writing
        >>> ga
        <zarr.v2.hierarchy.Group '/'>
        >>> gr = zarr.v2.open(store, mode='r')  # open existing group read-only
        >>> gr
        <zarr.v2.hierarchy.Group '/' read-only>
...
def save_array(store: StoreLike, arr, *, path=None, **kwargs):
    ...
    Save an array to a directory on the file system (uses a :class:`DirectoryStore`)::
        ...
        >>> import numpy as np
        >>> arr = np.arange(10000)
        >>> zarr.v2.save_array('data/example.zarr', arr)
        >>> zarr.v2.load('data/example.zarr')
        array([   0,    1,    2, ..., 9997, 9998, 9999])

    Save an array to a single file (uses a :class:`ZipStore`)::

        >>> zarr.v2.save_array('data/example.zip', arr)
        >>> zarr.v2.load('data/example.zip')
        array([   0,    1,    2, ..., 9997, 9998, 9999])
...
def save_group(store: StoreLike, *args, path=None, **kwargs):
    ...
    :class:`DirectoryStore`):
        ...
        >>> zarr.v2.save_group('data/example.zarr', a1, a2)
        >>> loader = zarr.v2.load('data/example.zarr')
        >>> loader
    ...
    Save several arrays using named keyword arguments::
        ...
        >>> zarr.v2.save_group('data/example.zarr', foo=a1, bar=a2)
        >>> loader = zarr.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
    ...
    Store several arrays in a single zip file (uses a :class:`ZipStore`)::
        ...
        >>> zarr.v2.save_group('data/example.zip', foo=a1, bar=a2)
        >>> loader = zarr.v2.load('data/example.zip')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
        array([   0,    1,    2, ..., 9997, 9998, 9999])
...
def save(store: StoreLike, *args, path=None, **kwargs):
    ...
    Save an array to a directory on the file system (uses a :class:`DirectoryStore`)::
        ...
        >>> import numpy as np
        >>> arr = np.arange(10000)
        >>> zarr.v2.save('data/example.zarr', arr)
        >>> zarr.v2.load('data/example.zarr')
        array([   0,    1,    2, ..., 9997, 9998, 9999])

    Save an array to a Zip file (uses a :class:`ZipStore`)::

        >>> zarr.v2.save('data/example.zip', arr)
        >>> zarr.v2.load('data/example.zip')
        array([   0,    1,    2, ..., 9997, 9998, 9999])
    ...
    :class:`DirectoryStore` and stores arrays in a group)::
        ...
        >>> a1 = np.arange(10000)
        >>> a2 = np.arange(10000, 0, -1)
        >>> zarr.v2.save('data/example.zarr', a1, a2)
        >>> loader = zarr.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: arr_0, arr_1>
        >>> loader['arr_0']
    ...
    Save several arrays using named keyword arguments::
        ...
        >>> zarr.v2.save('data/example.zarr', foo=a1, bar=a2)
        >>> loader = zarr.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
    ...
    Store several arrays in a single zip file (uses a :class:`ZipStore`)::
        ...
        >>> zarr.v2.save('data/example.zip', foo=a1, bar=a2)
        >>> loader = zarr.v2.load('data/example.zip')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
...
def copy_store(
    ...
    >>> import zarr
    >>> store1 = zarr.v2.DirectoryStore('data/example.zarr')
    >>> root = zarr.v2.group(store1, overwrite=True)
    ...
    >>> store2 = zarr.v2.ZipStore('data/example.zip', mode='w')
    >>> zarr.v2.copy_store(store1, store2, log=stdout)
    copy .zgroup
    copy foo/.zgroup
    copy foo/bar/.zgroup
...
def copy(
    ...
    **create_kws
    ...
    ``dry_run=True`` to find out what would happen without actually copying anything::
        ...
        >>> from sys import stdout
        >>> try:
        ...     zarr.v2.copy(source['foo'], dest, log=stdout, dry_run=True)
        ... except zarr.v2.CopyError as e:
        ...     print(e)
        ...
        copy /foo
        copy /foo/bar
        ...
        >>> zarr.v2.copy(source['foo'], dest, log=stdout, if_exists='replace', dry_run=True)
        copy /foo
        copy /foo/bar
        copy /foo/bar/baz (100,) int64
        ...
        (4, 0, 0)
        >>> zarr.v2.copy(source['foo'], dest, log=stdout, if_exists='skip', dry_run=True)
        copy /foo
        copy /foo/bar
        copy /foo/bar/baz (100,) int64
...
def copy_all(
    ...
    **create_kws
    ...
    >>> dest = zarr.v2.group()
    >>> import sys
    >>> zarr.v2.copy_all(source, dest, log=sys.stdout)
    copy /foo
    copy /foo/bar
    copy /foo/bar/baz (100,) int64
    copy /spam (100,) int64
...
def consolidate_metadata(store: BaseStore, metadata_key=".zmetadata", *, path=""):
    ...
    Returns
    -------
    g : :class:`zarr.v2.hierarchy.Group`
