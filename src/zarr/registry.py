        ...
        for e in self.lazy_load_list:
            ...
            if (hasattr(cls, "_zarr_spec_identifier") and hasattr(cls, "to_numpy")):
                for val in self.values():
                    # allow the V and B dtypes to be overloaded - something of an escape hatch.
                    if val.to_numpy == cls.to_numpy and cls.to_numpy.kind not in ["V", "B"]:
                        raise ValueError(
                            f"numpy dtype {cls.to_numpy} already exists for Zarr dtype {val}."
                        )

                self.register(cls, cls._zarr_spec_identifier)
            else:
                self.register(cls)

        self.lazy_load_list.clear()

    def register(self, cls: type[T], class_registry_key: str | None = None) -> None:
        if class_registry_key is None:
            self[fully_qualified_name(cls)] = cls
        else:
            if class_registry_key in self:
                raise ValueError(
                    f"{class_registry_key} already exists in the registry. Please try a different name."
                )  # not great, but don't want to have the possibility of clobbering existing core dtypes
            self[class_registry_key] = cls
...
def _collect_entrypoints() -> list[Registry[Any]]:
    ...
    __buffer_registry.lazy_load_list.extend(
        entry_points.select(group="zarr", name="buffer")
    )
    ...
    __v3_dtype_registry.lazy_load_list.extend(
        entry_points.select(group="zarr", name="v3dtype")
    )
    __v2_dtype_registry.lazy_load_list.extend(entry_points.select(group="zarr.v2dtype"))
    __v2_dtype_registry.lazy_load_list.extend(
        entry_points.select(group="zarr", name="v2dtype")
    )
    ...
    for group in entry_points.groups:
        if group.startswith("zarr.codecs."):
            ...
            __codec_registries[codec_name].lazy_load_list.extend(
...
def register_v2dtype(cls: type[ZarrDType]) -> None:
    assert cls.zarr_spec_format == "2"
    __v2_dtype_registry.register(cls, class_registry_key=cls._zarr_spec_identifier)
