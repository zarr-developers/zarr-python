...
from .storage import NestedDirectoryStore, _prog_ckey, _prog_number, FSStore, normalize_storage_path
...
class N5FSStore(FSStore):
    ...
    def __getitem__(self, key):
        if key.endswith(zarr_group_meta_key):
            ...
            key = key.replace(zarr_group_meta_key, self._META_KEYS[0])
            value = group_metadata_to_zarr(self._load_n5_attrs(key))
            ...
            key = key.replace(zarr_array_meta_key, self._META_KEYS[0])
            value = array_metadata_to_zarr(self._load_n5_attrs(key))
            ...
            key = key.replace(zarr_attrs_key, self._META_KEYS[0])
            value = attrs_to_zarr(self._load_n5_attrs(key))
    ...
    def listdir(self, path=None):
        ...
        if self._is_array(path):
            ...
            # replace n5 attribute file with respective zarr attribute files
            children.remove(self._META_KEYS[0])
            children.append(zarr_array_meta_key)
            if self._contains_attrs(path):
                children.append(zarr_attrs_key)
    ...
    def _is_group(self, path):
        ...
        if path is None:
            attrs_key = self._META_KEYS[0]
        else:
            attrs_key = os.path.join(path, self._META_KEYS[0])
    ...
    def _contains_attrs(self, path):
        ...
        if path is None:
            ...
            if not path.endswith(self._META_KEYS[0]):
                attrs_key = os.path.join(path, self._META_KEYS[0])
            else:  # pragma: no cover
                attrs_key = path
