...
- One unusable configuration member suppressed every other judgment
  about the same entity — a misspelled `index_location` hid the problems
  in a shard's inner pipelines. An optional member that fails its type
  check now falls back to absent, so the rest of the entity is still
  judged.
...
- A malformed `must_understand` — part of an entity's envelope, not of
  its configuration — counted as "the configuration is unreadable" and
