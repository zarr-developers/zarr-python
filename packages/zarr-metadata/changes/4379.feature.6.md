...
Every entry point that raises rejects it, the pydantic field types
included. The dedicated kind exists so that a consumer who wants the
tolerant reading can collect problems with `validate_*` and filter, and so
that an unknown key never masks the other findings about its entity.
