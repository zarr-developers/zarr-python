...
publishing JSON Schemas derived from the raw document TypedDicts. The instances
are the core model classes, so values interoperate freely with non-pydantic
code.
