...
def validate_array_metadata_v2(value: object) -> list[ValidationProblem]:
    ...
    if not isinstance(value, Mapping):
        return [ValidationProblem((), "expected a mapping", "invalid_type")]
    doc = cast("Mapping[str, object]", value)
    # Unlike the group document ("Other keys MUST NOT be present"), the v2
    # array document is open: other keys "SHOULD NOT be present within the
    # metadata object and SHOULD be ignored by implementations", so members
    # outside ARRAY_METADATA_STANDARD_KEYS_V2 are not problems.
