...
if importlib.util.find_spec("upath"):
    # Re-exported for zarr_storage.legacy._common, which needs it to recognize UPath store_like values.
...
from zarr_storage.legacy._abc import OffsetByteRequest, RangeByteRequest, SuffixByteRequest
...
if TYPE_CHECKING:
    ...
    from zarr_storage.legacy._abc import ByteRequest
...
def _join_paths(paths: Iterable[str]) -> str:
    ...
    --------
    ```python
    from zarr_storage.legacy._utils import _join_paths
    _join_paths(["", "a", "b"])
    # 'a/b'
    _join_paths(["a", "b", "c"])
    # 'a/b/c'
...
def _dereference_path(root: str, path: str) -> str:
    ...
    --------
    ```python
    from zarr_storage.legacy._utils import _dereference_path
...
def _relativize_path(*, path: str, prefix: str) -> str:
    ...
    --------
    ```python
    from zarr_storage.legacy._utils import _relativize_path
