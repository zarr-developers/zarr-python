...
from zarr_storage.legacy import LocalStore
from zarr_storage.legacy._local import _atomic_write
...
def test_atomic_write_exclusive_preexisting(tmp_path: pathlib.Path) -> None:
    ...
    with path.open("wb") as f:
        f.write(b"xyz")
    assert path.read_bytes() == b"xyz"
    with pytest.raises(FileExistsError), _atomic_write(path, "wb", exclusive=True) as f:
        f.write(b"abc")
    assert path.read_bytes() == b"xyz"
    assert list(path.parent.iterdir()) == [path]  # no temp files
