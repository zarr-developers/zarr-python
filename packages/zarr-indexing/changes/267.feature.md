...
`__getitem__` taking integers and slices — is enough. A `numpy.ma` source keeps
...
its box. `with_parts` accepts `(0,)` for a zero-length axis, as it already
accepted `()`.
